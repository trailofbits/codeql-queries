...
predicate isChecked(Expr value) {
  ...
  exists(IfStmt is |
    DataFlow::localFlow(DataFlow::exprNode(value),
  ...
  exists(SwitchStmt ss |
    DataFlow::localFlow(DataFlow::exprNode(value), DataFlow::exprNode(ss.getExpr().getAChild*()))
  )
}

from ErrorCode res
where not isChecked(res)
select res.getLocation(),
