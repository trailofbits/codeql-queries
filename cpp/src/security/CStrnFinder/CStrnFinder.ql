...
class StrNFunctionCallTwoSources extends StrNFunction {
  ...
  override int getSizeArg() { result = 2 }
}
...
class StrNFunctionCallSingleSource extends StrNFunction {
  ...
  override int getSourceArgs() { if this.getName() in ["bcopy"] then result = 0 else result = 1 }

  override int getSizeArg() {
    if this.getName() in ["memccpy", "__builtin___memccpy_chk"] then result = 3 else result = 2
  }
}
...
int getKnownInt(Expr argument) { result = argument.getValue().toInt() }
...
int getKnownLen(Expr sourceArg) {
  ...
  exists(LocalScopeVariable v, SsaDefinition ssaDef |
    sourceArg = ssaDef.getAUse(v) and
    result = ssaDef.getAnUltimateDefiningValue(v).(StringLiteral).getOriginalLength() - 1
  )
}
...
where
  ...
  sourceSize =
    min(int sourceSizeTmp |
      sourceSizeTmp = getKnownLen(fc.getArgument(f.getSourceArgs()))
    |
      sourceSizeTmp
    ) and
  // get size argument
  sizeArgValue = getKnownInt(fc.getArgument(f.getSizeArg())) and
  (
    // typo bug
    // for "testabc" report sizes 6 and 5; 7 and greater sizes are fine; greater typos are probably intended
    sourceSize - sizeArgValue = [1, 2] and
    msg = "Not whole source buffer is consumed, which may be a bug."
    or
    (
      sizeArgValue - 1 > sourceSize and
      if f.getName().matches("%mem%") or f.getName() in ["RtlCopyMemoryNonTemporal", "bcopy"]
      then msg = "This is a buffer overread vulnerability."
      else
        if f.getName().matches("%cat%")
        then
          msg =
            "This indicates that the destination buffer may overflow, as the size argument limits amount of bytes copied, and not total length."
        else none()
    )
  )
select fc,
  "Call to " + f.getName() + " receives an input string of size " + sourceSize +
