...
class StaticKeyLiteral extends ArrayOrVectorAggregateLiteral {
  ...
  StaticKeyLiteral() { this = var.getInitializer().getExpr() }
...
class StaticKeyAccess extends VariableAccess {
  ...
  StaticKeyAccess() { this = key.getAnAccess() }

  StaticKeyLiteral getKey() { result = key }
}

class StaticPasswordLiteral extends StringLiteral {
  VariableAccess getAnAccess() { result = this.getEnclosingVariable().getAnAccess() }
}

class StaticPasswordAccess extends VariableAccess {
  StaticPasswordLiteral password;

  StaticPasswordAccess() { this = password.getAnAccess() }

  StaticPasswordLiteral getPassword() { result = password }
}

module StaticKeyConfig implements DataFlow::ConfigSig {
  predicate isSource(DataFlow::Node source) { source.asExpr() instanceof StaticKeyAccess }

  predicate isSink(DataFlow::Node sink) { sink.asExpr() instanceof StrongRandomnessSink }
}

module StaticKeyFlow = DataFlow::Global<StaticKeyConfig>;

module StaticPasswordConfig implements DataFlow::ConfigSig {
  predicate isSource(DataFlow::Node source) { source.asExpr() instanceof StaticPasswordAccess }

  predicate isSink(DataFlow::Node sink) { sink.asExpr() instanceof StrongPasswordSink }
}
