...
where
  // get a FileMode
  fileModeExpr.getType().getName() = "FileMode"

  // go over all numeric constants in expressions like `013 | 37 & os.ModeSticky`
  and fileModeLit.getParent*() = fileModeExpr

  // get integer and string values of the mode
  and fileModeLit.getIntValue() = fileModeInt
  and fileModeLitStr = fileModeLit.toString()

  // skip reporting if a popular decimal constant is used
  and not isKnownValidConstant(fileModeLitStr)
  ...
  // chmod
  and if fileModeInt > 511 /* 0o777 */ and exists(PermChangingMethod use | use.getAnArgument() = fileModeExpr)
  then
      message = "- only 9 permission bits are supported in Mkdir/MkdirAll/Chmod methods on all platforms"
  else (
    // skip reporting if the mode is large - probably a mask
    not fileModeInt > 1911 /* 0x777 */
    
    // the most interesting case of number encoding typos
    and exists(string fileModeAsOctal, string fileModeAsSeen |
      ...
      and message = "will evaluate to 0o" + fileModeAsOctal
    )
  )
