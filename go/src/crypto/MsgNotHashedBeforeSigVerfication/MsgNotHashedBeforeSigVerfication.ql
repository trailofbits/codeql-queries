...
class SignatureMsgTruncationFunction extends Function {
  ...
  SignatureMsgTruncationFunction() {
    ...
    or
    this.hasQualifiedName("github.com/btcsuite/btcd/btcec.PrivateKey", "Sign") and
    msgHashPosition = 0
    or
    this.hasQualifiedName("github.com/btcsuite/btcd/btcec.PublicKey", "RecoverCompact") and
    msgHashPosition = 2
    or
    this.hasQualifiedName("github.com/btcsuite/btcd/btcec.Signature", "Verify") and
    msgHashPosition = 0
    or
    // https://github.com/btcsuite/btcd/blob/2f508b3f86ed9ef87bcf3426b87b6c0dc0d3632c/btcec/signature.go#L256-L263
    this.hasQualifiedName("github.com/umbracle/ethgo/wallet", "Ecrecover") and msgHashPosition = 0
    or
    this.hasQualifiedName("github.com/umbracle/ethgo/wallet", "RecoverPubkey") and
    msgHashPosition = 1
    or
    ...
    exists(string dcrdVersion | dcrdVersion = ["", "/v2", "/v3", "/v4"] |
      this.hasQualifiedName("github.com/decred/dcrd/dcrec/secp256k1" + dcrdVersion,
        ["RecoverCompact", "RecoverCompact", "Sign"]) and
      msgHashPosition = 1
      or
      this.hasQualifiedName("github.com/decred/dcrd/dcrec/secp256k1" + dcrdVersion + ".Signature",
        "Verify") and
      msgHashPosition = 0
    )
  ...
  predicate hashArgPosition(int position) { msgHashPosition = position }
}
  ...
  HashFunction() {
    this.getName()
        ...
        .matches([
            ...
            "ECDSA384", "ES512", "ECDSA512", "SHA2", "SHA224", "RIPEMD320", "SHA0", "HAVEL128",
            "MD2", "SHA1", "MD4", "MD5", "PANAMA", "RIPEMD", "RIPEMD128", "RIPEMD256", "ARGON2",
            "PBKDF2", "BCRYPT", "SCRYPT", "KECCAK256", "KECCAK256RLP", "HASH"
          ])
  }
}
...
module AnythingToSignatureMsgTrunFuncConfig implements DataFlow::ConfigSig {
  ...
  predicate isSource(DataFlow::Node source) {
    not isSink(source) and
    not source.asInstruction() instanceof IR::ReadArgumentInstruction
  }

  predicate isSink(DataFlow::Node sink) {
    exists(SignatureMsgTruncationFunction sigUseF, CallExpr sigUseCall, int position |
      sigUseCall.getTarget() = sigUseF and
      sigUseF.hashArgPosition(position) and
      sink.asExpr() = sigUseCall.getArgument(position)
    )
  }
  ...
  predicate isBarrier(DataFlow::Node node) {
    ...
    exists(HashFunction hf |
      hf.getACall().getResult(_) = node or hf.getACall().getArgument(_) = node
    )
    or
...
from
  AnythingToSignatureMsgTrunFuncFlow::PathNode source,
  AnythingToSignatureMsgTrunFuncFlow::PathNode sink
where
  AnythingToSignatureMsgTrunFuncFlow::flowPath(source, sink) and
  // only the longest flow
  // TODO: find only flows originating from user input
  not exists(DataFlow::Node source2 |
    LongestFlowFlow::flow(source2, source.getNode()) and
    source2 != source.getNode()
  )
